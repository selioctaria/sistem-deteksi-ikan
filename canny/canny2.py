import cv2
import os
import numpy as np
import csv

# =========================
# KONFIGURASI PARAMETER
# =========================
input_root = r"D:\Documents\SKRIPSI\PROGRAM SISTEM DETEKSI IKAN\dataset\augmented"
output_root = r"D:\Documents\SKRIPSI\PROGRAM SISTEM DETEKSI IKAN\canny\citra_canny"
csv_path = r"D:\Documents\SKRIPSI\PROGRAM SISTEM DETEKSI IKAN\canny\hasil_piksel_tepi.csv"
csv_avg_path = r"D:\Documents\SKRIPSI\PROGRAM SISTEM DETEKSI IKAN\canny\hasil_piksel_tepi_rata_rata.csv"

# Gaussian Blur
gaussian_kernel = (5, 5)
gaussian_sigma = 1.4

# Canny Threshold
low_threshold = 50
high_threshold = 150

os.makedirs(output_root, exist_ok=True)

# =========================
# SIAPKAN FILE CSV UTAMA
# =========================
with open(csv_path, mode='w', newline='') as file:
    writer = csv.writer(file)
    writer.writerow([
        "Kelas",
        "Nama File",
        "Jumlah Piksel Tepi",
        "Total Piksel",
        "Rasio Piksel Tepi"
    ])

# =========================
# SIAPKAN FILE CSV RATA-RATA
# =========================
with open(csv_avg_path, mode='w', newline='') as file_avg:
    writer_avg = csv.writer(file_avg)
    writer_avg.writerow([
        "Kelas",
        "Nama File",
        "Jumlah Piksel Tepi",
        "Total Piksel",
        "Rasio Piksel Tepi"
    ])

# =========================
# PROSES DATASET
# =========================
for class_name in os.listdir(input_root):
    class_input_path = os.path.join(input_root, class_name)
    if not os.path.isdir(class_input_path):
        continue

    class_output_path = os.path.join(output_root, class_name)
    os.makedirs(class_output_path, exist_ok=True)

    print(f"\n📁 Memproses kelas: {class_name}")

    # List untuk menyimpan nilai edge tiap file
    edge_pixels_list = []
    edge_ratio_list = []
    total_pixels_list = []

    for file in os.listdir(class_input_path):
        if file.lower().endswith(".png"):
            img_path = os.path.join(class_input_path, file)

            # Baca citra grayscale
            img = cv2.imread(img_path, cv2.IMREAD_GRAYSCALE)
            if img is None:
                print(f"❌ Gagal membaca {file}")
                continue

            # 1. Gaussian Smoothing
            blurred = cv2.GaussianBlur(img, gaussian_kernel, gaussian_sigma)

            # 2. Canny Edge Detection
            edges = cv2.Canny(blurred, low_threshold, high_threshold)

            # 3. Hitung jumlah piksel tepi
            edge_pixels = np.sum(edges > 0)
            total_pixels = edges.size
            edge_ratio = edge_pixels / total_pixels

            # Simpan nilai ke list
            edge_pixels_list.append(edge_pixels)
            edge_ratio_list.append(edge_ratio)
            total_pixels_list.append(total_pixels)

            # 4. Simpan hasil citra
            output_path = os.path.join(class_output_path, file)
            cv2.imwrite(output_path, edges)

            # 5. Simpan ke CSV utama
            with open(csv_path, mode='a', newline='') as file_csv:
                writer = csv.writer(file_csv)
                writer.writerow([
                    class_name,
                    file,
                    edge_pixels,
                    total_pixels,
                    round(edge_ratio, 6)
                ])

            print(f"✅ {class_name}/{file} | Edge: {edge_pixels} | Rasio: {edge_ratio:.4f}")

    # =========================
    # Hitung rata-rata per kelas
    # =========================
    if edge_pixels_list:
        avg_edge_pixels = int(np.mean(edge_pixels_list))
        avg_edge_ratio = float(np.mean(edge_ratio_list))
        avg_total_pixels = int(np.mean(total_pixels_list))  # biasanya sama untuk semua citra

        print(f"\n📊 Rata-rata untuk kelas {class_name}:")
        print(f"   Jumlah Piksel Tepi: {avg_edge_pixels}")
        print(f"   Rasio Piksel Tepi: {avg_edge_ratio:.6f}")

        # Simpan rata-rata ke CSV rata-rata
        with open(csv_avg_path, mode='a', newline='') as file_avg:
            writer_avg = csv.writer(file_avg)
            writer_avg.writerow([
                class_name,
                "RATA-RATA",
                avg_edge_pixels,
                avg_total_pixels,
                round(avg_edge_ratio, 6)
            ])